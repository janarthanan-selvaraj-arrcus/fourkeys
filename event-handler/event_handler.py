...
def bulk():
    ...
    if not request.is_json:
        abort(404, "Invalid request body: Not a valid json")

    body_json = request.get_json()
    if type(body_json) != type([]):
        abort(404, "Invalid request body: Request body is not json array")
    
    # Publish to Pub/Sub
    for i in body_json:
        pubsub_headers = dict(request.headers)
        pubsub_headers["X-Github-Event"] = i["meta_data"]["event_type"]

        pubsub_source = i["meta_data"]["source"]

        pubsub_body = bytes(json.dumps(i["body"]), 'utf-8')

        publish_to_pubsub(pubsub_source, pubsub_body, pubsub_headers)
